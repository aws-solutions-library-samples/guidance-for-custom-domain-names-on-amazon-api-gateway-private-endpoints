import { elbTypeEnum, proxyDomain } from '../bin/Main'
...
export class FargateServiceConstruct extends Construct {
  constructor (scope: Construct, id: string, props: FargateProps) {
    ...
    const customResourceProvider = new cr.Provider(
      this,
      `${stackName}-CRProvider-interfaceDNS`,
      {
    ...
    const apiGatewayVPCInterfaceEndpointDNSName = objCustomResource
      .getAtt('Result')
      .toString()
    ...
    const fgContainer = new ecs.ContainerDefinition(
      ...
      `${stackName}-container-def`,
      {
        ...
        healthCheck: {
          command: [
            'CMD-SHELL',
            'curl --cacert /cert.pem https://localhost || exit 1'
          ]
        }
      }
    )
    ...
    if (props.elbType === elbTypeEnum.NLB) {
      service.attachToNetworkTargetGroup(
        props.targetGroup as NetworkTargetGroup
      )
    } else {
      service.attachToApplicationTargetGroup(
        props.targetGroup as ApplicationTargetGroup
      )
    }
