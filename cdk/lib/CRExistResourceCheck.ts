...
export const handler = async (
  ...
  try {
    ...
    const subnetIds = externalPrivateSubnetIds
      ? JSON.parse(externalPrivateSubnetIds)
      : undefined

    console.log(`subnetIds->-->${subnetIds}`)

    if (event.RequestType === 'Create' || event.RequestType === 'Update') {
      endpointsCreated = await handleCreateOrUpdate(
        vpcId,
        subnetIds,
        ExternalEndpointSg
      )
      ...
      if (event.RequestType === 'Update' && physicalResourceID) {
        const vpcEndPoints = JSON.parse(
          event.PhysicalResourceId
        ) as IEndpointsCreated
      ...
      const endPointsCreated = JSON.parse(
        event.PhysicalResourceId
      ) as IEndpointsCreated
...
const handleCreateOrUpdate = async (
  ...
  const vpcEndpoints: ec2.DescribeVpcEndpointsCommandOutput = await client.send(
    command
  )
    ...
    const endpointFound = vpcEndpoints.VpcEndpoints?.find(
      (endpoint: VpcEndpoint) =>
        endpoint.ServiceName ===
          `com.amazonaws.${region}.${interfaceEndpoint}` &&
        (endpoint.State === 'available' || endpoint.State === 'pending')
    )

    if (endpointFound) {
      console.log(
        `endpointFound22222---> ${JSON.stringify(endpointFound.ServiceName)}`
      )

      if (
        interfaceEndpoint === 's3' &&
        endpointFound.VpcEndpointType === 'Interface'
      ) {
        console.log(
          `S3 endpoint gateway not found , so creating---> ${JSON.stringify(
            interfaceEndpoint
          )}`
        )
        ...
        if (
          ...
          console.log(
            `Private DNS is Disabled on ${interfaceEndpoint}, enabling now....`
          )

          const ModifyVpcEndpointCommandResp = await client.send(
            ModifyVpcEndpointCommand
          )
          console.log(
            `ModifyVpcEndpointCommandResp---> ${JSON.stringify(
              ModifyVpcEndpointCommandResp
            )}`
          )
        }

        console.log(
          `endpointFound444444---> ${JSON.stringify(
            endpointFound.ServiceName
          )}`
        )
        allEndpoints[interfaceEndpoint] = {
...
const createEndpoint = async (
  ...
  const command = new ec2.CreateVpcEndpointCommand({
    ...
    RouteTableIds:
      type === 'Gateway'
        ? await getPrivateRouteTableIds(vpcId, externalPrivateSubnetIds)
        : undefined,
