...
export const handler = async (event: CloudFormationCustomResourceEvent) => {
  ...
  try {
    ...
    if (RequestType === 'Create' || RequestType === 'Update') {
      ...
      const vpcEndpointsObj: ec2.DescribeVpcEndpointsCommandOutput =
        await client.send(command)
      let timer = 0
      while (
        typeof vpcEndpointsObj === 'undefined' ||
        ...
        typeof vpcEndpointsObj.VpcEndpoints[0].DnsEntries[0].DnsName ===
          'undefined'
      ) {
        if (timer >= 300) {
          console.error(
            'The required object values were not found within 300 seconds.'
          )
          break
        }
        timer++
      ...
      if (
        vpcEndpointsObj?.VpcEndpoints &&
        vpcEndpointsObj.VpcEndpoints[0]?.DnsEntries &&
        vpcEndpointsObj.VpcEndpoints[0].DnsEntries[0]?.DnsName
      ) {
        apiGatewayVpcDNSName =
          vpcEndpointsObj.VpcEndpoints[0].DnsEntries[0].DnsName
        console.log('API Gateway VPC DNS name:', apiGatewayVpcDNSName)
      } else {
        console.error(
          'API Gateway VPC DNS name, The required object values were not found within 300 seconds.'
        )
      }
