...
ENDPOINT_URL=$(echo $NGINX_CONFIG | grep -e '\$endpointUrl .*;' | awk '{print$3}' | awk -F";" '{print$1}' |head -1)
SERVER_NAMES=$(echo $NGINX_CONFIG | grep -e '.*server_name .*;' | awk '{print$2}' | awk -F';' '{print$1}')
