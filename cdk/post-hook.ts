...
const GenerateOutputsFile = (
  ...
  proxyDomains.forEach((item) => {
    ...
    if (api_gateway_id in arrPolicyMappings) {
      ...
      arrPolicyMappings[api_gateway_id].Statement.push(
  ...
  ------> outputs.json file saved at location ${path.join(
...
const generateSpecificPolicyStatement = (
  props: IArguments,
  item: proxyDomain,
) => {
  return {
  ...
  if (policyType === "allow") {
    if (item.VERBS) {
      item.VERBS.forEach((verb) => {
        resources.push(
          `execute-api:/${stage}/${verb.toUpperCase()}/${resourcePath}`,
        );
      });
    } else {
