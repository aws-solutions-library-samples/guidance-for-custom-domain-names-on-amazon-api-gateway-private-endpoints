...
export type proxyDomain = {
  ...
  PRIVATE_ZONE?: IPrivateHostedZone;
...
const proxyDomains: proxyDomain[] = JSON.parse(
...
const externalPrivateSubnetIds: string =
  process.env.EXTERNAL_PRIVATE_SUBNETS_ID || ''
...
const taskImage: string =
  (process.env.TASK_IMAGE || 'public.ecr.aws/nginx/nginx') +
  ':' +
  (process.env.TASK_IMAGE_TAG || '1.23-alpine-perl')
...
const taskScaleCpuPercentage = Number(
  process.env.TASK_SCALE_MAX || '80'
)
