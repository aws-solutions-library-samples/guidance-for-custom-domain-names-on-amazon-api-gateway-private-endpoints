...
const Run = async () => {
  ...
  if (!domainsList) {
    throw new Error(
      "At least one domain needs to be in proxy-config file to add SSL certificate to load balancer listener.",
    );
  }
...
const GetZoneID = function (
  customDomain: string,
  publicZones: route53_sdk.HostedZone[],
): string {
  let zoneId;
  const extractDm = extractDomain(customDomain, { tld: false });
  // console.log( `extractDm--> ${ extractDm }` )
  let probableBaseDomain =
    customDomain === extractDm
      ? customDomain
      : customDomain.substring(customDomain.indexOf(".") + 1);
  ...
  while (probableBaseDomain !== tld) {
    ...
    if (zone) {
      ...
      probableBaseDomain = probableBaseDomain.substring(
        probableBaseDomain.indexOf(".") + 1,
      );
    }
  }
